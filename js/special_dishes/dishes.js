...
const renderDishes = async () => {
	let specialDishId = $("#specialDishId").text();
	const dishes = await get(`special-dishes/${specialDishId}/dishes`);
	let tbody = "";
...
const addDishesToCombo = async (dishes) => {
	...
	const response = await post(`special-dishes/${specialDishId}/dishes`, data);
...
const loadDelete = async (dishId, name) => {
	Swal.fire({
		...
		if (result.isConfirmed) {
			let specialDishId = $("#specialDishId").text();
			let url = `special-dishes/${specialDishId}/dishes/${dishId}`;
