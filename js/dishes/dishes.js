...
$('#editFoodForm').submit((e) => {
	...
	let quantityNotify = $('#foodQuantityNotify').val();
	let cost = $('#foodCost').val();
	let piecesPerPackage = $('#foodPiecesPerPackage').val();
	let showInIndex = $('#foodShowInIndex').is(':checked') ? 1 : 0;

	$('#editModal').modal('hide');
	editFood(foodId, name, quantityNotify, cost, piecesPerPackage, showInIndex);
});

const editFood = async (foodId, name, quantityNotify, cost, piecesPerPackage, showInIndex) => {
	...
	if (response.statusCode == 200) {
		...
		$('#foodName').val(name);
		$('#foodQuantityNotify').val(quantityNotify);
		$('#foodCost').val(cost);
		$('#foodPiecesPerPackage').val(piecesPerPackage);
